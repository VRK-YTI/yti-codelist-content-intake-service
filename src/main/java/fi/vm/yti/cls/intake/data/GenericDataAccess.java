...
public class GenericDataAccess implements DataAccess {
    ...
    private final BusinessServiceSubRegionParser businessServiceSubRegionParser;
    ...
    public GenericDataAccess(final Domain domain,
        ...
        this.businessServiceSubRegionParser = businessServiceSubRegionParser;
    }
    ...
    private void loadDefaultMunicipalities() {
        ...
        if (updateManager.shouldUpdateData(DomainConstants.DATA_MUNICIPALITIES, DEFAULT_MUNICIPALITY_FILENAME)) {
            ...
            try (final InputStream inputStream = FileUtils.loadFileFromClassPath("/municipalities/" + DEFAULT_MUNICIPALITY_FILENAME)) {
                final List<Municipality> municipalities = municipalityParser.parseMunicipalitiesFromInputStream(DomainConstants.SOURCE_AVOINDATA, inputStream);
    ...
    private void loadDefaultMagistrates() {
        ...
        if (updateManager.shouldUpdateData(DomainConstants.DATA_MAGISTRATES, DEFAULT_MUNICIPALITY_FILENAME)) {
            final UpdateStatus updateStatus = updateManager.createStatus(DomainConstants.DATA_MAGISTRATES, DomainConstants.SOURCE_AVOINDATA, DEFAULT_MUNICIPALITY_FILENAME, UPDATE_RUNNING);
            final Stopwatch watch = Stopwatch.createStarted();
            try (final InputStream inputStream = FileUtils.loadFileFromClassPath("/municipalities/" + DEFAULT_MUNICIPALITY_FILENAME)) {
                final List<Magistrate> magistrates = magistrateParser.parseMagistratesFromInputStream(DomainConstants.SOURCE_AVOINDATA, inputStream);
    ...
    private void loadDefaultRegions() {
        ...
        if (updateManager.shouldUpdateData(DomainConstants.DATA_REGIONS, DEFAULT_MUNICIPALITY_FILENAME)) {
            final UpdateStatus updateStatus = updateManager.createStatus(DomainConstants.DATA_REGIONS, DomainConstants.SOURCE_AVOINDATA, DEFAULT_MUNICIPALITY_FILENAME, UPDATE_RUNNING);
            final Stopwatch watch = Stopwatch.createStarted();
            try (final InputStream inputStream = FileUtils.loadFileFromClassPath("/municipalities/" + DEFAULT_MUNICIPALITY_FILENAME)) {
                final List<Region> regions = regionParser.parseRegionsFromInputStream(DomainConstants.SOURCE_AVOINDATA, inputStream);
